...
import classes from "./dssert.module.scss";
import CustomizedSnackbars from "../../components/Snackbar/index.jsx";
import Divider from '@mui/material/Divider';
import AddDssert from './addDssert';
...
function descendingComparator(a, b, orderBy) {
  ...
  const headCells = [
    ...
    { id: "name", numeric: false, disablePadding: true, label: "Dssert Name" },
    {
      id: "halfPrice",
      numeric: true,
...
const Dssert = () => {
    ...
    const handleAddDssert = () => {
      setMode("add");
      setOpen(true);
      setNewCourse({ name: "", halfPrice: "", fullPrice: "", imageFile: null });
    };
      ...
      try {
        if (mode === "edit") {
          ...
          if (response.status === 200) {
            setSnackbarState({
              open: true,
              message: "Dssert Added Successfully!",
              severity: "success",
            });
            setRefreshData((prev) => !prev); // Refresh the data
          } else {
    ...
    useEffect(() => {
      const fetchData = async () => {
        setIsLoading(true);
        try {
          const response = await axiosInstanceToken.get("getDssertList");
