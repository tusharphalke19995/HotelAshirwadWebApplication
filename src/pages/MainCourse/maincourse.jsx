...
import classes from "./maincourse.module.scss";
import CustomizedSnackbars from "../../components/Snackbar/index.jsx";
import Divider from '@mui/material/Divider';
import AddMainCourse from "./addMainCourse.jsx";
...
function descendingComparator(a, b, orderBy) {
  ...
  const headCells = [
    ...
    { id: "crName", numeric: false, disablePadding: true, label: "Main Course Name" },
    {
  ...
  function EnhancedTableToolbar({ onAddClick }) {
    return (
      <Toolbar sx={{ pl: { sm: 2 }, pr: { xs: 1, sm: 1 } }}>
        ...
        <Button
          variant="contained"
          className={classes.buttonAddMainCourse}
          onClick={onAddClick}
        >
          Add MainCourse
        </Button>
...
const MainCourse = () => {
    ...
    const handleAddMainCourse = () => {
      setMode("add");
      setOpen(true);
      setNewCourse({ crName: "",category:"", halfPrice: "", fullPrice: "", imageFile: null });
    };
  
    const handleEditMainCourse = (data) => {
      setMode("edit");
      setNewCourse(data);
      setOpen(true);
    };
      ...
      try {
        if (mode === "edit") {
          ...
          if (response.status === 200) {
            ...
            throw new Error("Failed to update main course data. Please try again.");
          }
        } else {
          const formData = new FormData();
          ...
          if (response.status === 200) {
            ...
            throw new Error("Failed to add add main course data. Please try again.");
          }
        }
      } catch (error) {
        toast.error(
          `Failed to ${
            mode === "add" ? "Add" : "Update"
          } main course data. Please try again.`
        );
    ...
    useEffect(() => {
      const fetchData = async () => {
        setIsLoading(true);
        try {
          const response = await axiosInstanceToken.get("getMainCourseList");
          const data = response.data || [];
          setMainCourseList(data);
        } catch (error) {
          toast.error("Failed to load main course data.");
    ...
    const emptyRows =
      page > 0 ? Math.max(0, (1 + page) * rowsPerPage - mainCourseList.length) : 0;
  
    const visibleRows = useMemo(() => {
      return stableSort(mainCourseList, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      );
    }, [mainCourseList, order, orderBy, page, rowsPerPage]);
  
    return (
      <Box sx={{ width: "100%" }}>
        <Paper sx={{ width: "100%", mb: 2 }}>
          <EnhancedTableToolbar onAddClick={handleAddMainCourse} />
          <Divider />
          <TableContainer className={classes.tableData}>
            ...
            >
              <EnhancedTableHead
                ...
                rowCount={mainCourseList.length}
              />
              <TableBody>
                {visibleRows.map((row, index) => {
                  ...
                  return (
                    <TableRow
                      ...
                      tabIndex={-1}
                      sx={{ cursor: "pointer" }}
                      onClick={() => handleEditMainCourse(row)}
                    >
